...
class HistoryManage {
  ...
  initComponentRecord(component: Component) {
    ...
    if (component instanceof Collection) {
      component.children.forEach((item) => this.initComponentRecord(item));
    }
  }
  ...
  createRecord(start: Cursor, end: Cursor) {
    ...
    for (let i = this.nowStackIndex + 1; i < this.recordStack.length; i++) {
      ...
      componentList.forEach((item) => {
        item.record.clear(this.nowStackIndex + 1);
      });
    }
    // 清除全局历史栈中无效的历史
  ...
  undo() {
    ...
    for (let i = 0; i < nowRecord.componentList.length; i++) {
      ...
      item.record.restore(this.nowStackIndex - 1);
    }
    ...
    focusAt(
      ...
      nowRecord.selection.start,
      nowRecord.selection.end,
    );
  }
